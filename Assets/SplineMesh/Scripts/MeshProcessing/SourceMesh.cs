...
namespace SplineMesh {
    ...
    public struct SourceMesh {
        // Rueckfallwerte, solange BuildData() mangels lesbarem Mesh nichts aufbauen konnte.
        private static readonly List<MeshVertex> s_NoVertices = new List<MeshVertex>();
        private static readonly int[] s_NoTriangles = new int[0];
        ...
        internal int[] Triangles {
            get {
                if (vertices == null) BuildData();
                return triangles ?? s_NoTriangles;
            }
        }
        ...
        private void BuildData() {
            // Ohne "Read/Write Enabled" liefert Unity hier leere Arrays. Diesen Zustand
            // duerfen wir nicht cachen: der Cache wird nur verworfen, wenn sich Mesh,
            // translation, rotation oder scale aendern (siehe Equals). Ein spaeterer
            // Reimport mit aktiviertem Read/Write wuerde sonst nie greifen - stattdessen
            // setzt MeshBender 0 Vertices gegen volle UV-Arrays, und Unity meldet nur noch
            // "Mesh.uv is out of bounds", was die eigentliche Ursache verschleiert.
            // Deshalb hier bewusst nichts zwischenspeichern: der naechste Zugriff versucht es erneut.
            if (Mesh == null || Mesh.vertexCount == 0) {
                Debug.LogError("SourceMesh: Mesh '" + (Mesh == null ? "<null>" : Mesh.name) +
                    "' liefert keine Vertices. Bei importierten Modellen 'Read/Write Enabled' " +
                    "in den Import-Einstellungen aktivieren.");
                return;
            }
